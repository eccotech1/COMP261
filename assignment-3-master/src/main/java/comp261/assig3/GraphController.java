...
import comp261.assig3.FordFulkerson.vehicles;
...
public class GraphController {
    ...
    boolean isBus = false;
    boolean isCar = false;
    boolean isMoped = false;

    //weight of vehicle default = 1
    public static double vehicleWeight = 1;
    ...
    //handle bus rBtn
    public void handleBus(ActionEvent event) {
        if(bus.isSelected() == true){
            isBus = true;
            vehicleWeight = 1;
            moped.setDisable(true);;
            car.setDisable(true);
            event.consume();
        }
        else{
    ...
    //handle moped rBtn
    public void handleMoped(ActionEvent event) {
        if(moped.isSelected() == true){
            isMoped = true;
            car.setDisable(true);
            bus.setDisable(true);
            vehicleWeight = 4;
    ...
    */
    ...
    public void handleNetworkFlow(ActionEvent event) {
        ...
        tripText.appendText("\nSource: " + graph.getNodeList().get(0).getName() + " Sink: "
    ...
    public void handleAugmentationPaths(ActionEvent event) {
        ...
        double maxFlow = f.calcMaxflows(graph, graph.getSource(), graph.getSink());
        ArrayList<Pair<ArrayList<Node>, Double>> aPaths = new ArrayList<Pair<ArrayList<Node>, Double>>();
        aPaths = f.getAugmentationPaths();
        StringBuilder sb = new StringBuilder();
        if (aPaths != null) {
            tripText.appendText("\nAP: ");

            for(Pair<ArrayList<Node>,Double> path : aPaths){
                tripText.appendText(" "+path.getKey().toString().substring(1,path.getKey().toString().length()-1)+"  Flow Value: "+ path.getValue().toString() + " || ");
            }
    ...
    public void handleMinCut(ActionEvent event) {
        ...
        //arraylist of source and sink for printing
        ArrayList<String> source = new ArrayList<String>();
        ArrayList<String> sink = new ArrayList<String>();

        
        //Display the min-cut sets and the corresponding capacity
        if (minCutwithSets != null) {
            //Display the min-cut sets and the corresponding capacity
            //source node
            for(Node sourceNode: minCutwithSets.getKey().getKey()){
               source.add(sourceNode.getName());
            } 
            tripText.appendText("Source: "+source+"\n");
            //sink node
            for(Node sinkNode: minCutwithSets.getKey().getValue()){
                sink.add(sinkNode.getName());
            }
            //print sink
            tripText.appendText("Sink: "+sink+"\n");
            double n = minCutwithSets.getValue();
            
            tripText.appendText("Min-cut: "+ n + "\n");
