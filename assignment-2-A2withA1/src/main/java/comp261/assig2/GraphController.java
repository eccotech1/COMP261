...
public class GraphController {
    ...
    @FXML
    private CheckBox Timecost_ch;
    @FXML
    private CheckBox Distancecost_ch;
    @FXML
    ...
    public static boolean costTime = true;
    ...
    public void initialize() {
        ...
        Distancecost_ch.setDisable(true);
        Timecost_ch.setSelected(true);
    ...
    public void handleStartAction(ActionEvent event) {
        ...
        pathEdges = AStar.findShortestPathEdges(graph, startLocation, goalLocation, costTime);
    ...
    public void handleShowConnectedComponents(ActionEvent event) {
        ...
        System.out.println(graph.findComponents());
        drawGraph(graph);
    }
    ...
    // handlAddDistance calculates the cost using distance
    public void handleAddDistance(ActionEvent event) {

        if (Distancecost_ch.isSelected()) { //check if selected
            Timecost_ch.setDisable(true); //disable
            Timecost_ch.setSelected(false); // not selected
            costTime = false; //cost distance
            pathEdges = AStar.findShortestPathEdges(graph, startLocation, goalLocation, costTime); //update
            drawGraph(graph);
            event.consume();
        } else {
            Timecost_ch.setDisable(false); //enable
        }


    }
    // handleAddTime calculates the cost using time
    public void handleAddTime(ActionEvent event) {
        if (Timecost_ch.isSelected()) { //check if selected
            Distancecost_ch.setDisable(true); //disable
            Distancecost_ch.setSelected(false); //not selected
            costTime = true; //cost distance
            pathEdges = AStar.findShortestPathEdges(graph, startLocation, goalLocation, costTime); //update
            drawGraph(graph);
            event.consume();
        } else {
            Distancecost_ch.setDisable(false); //enable
        }
    }
    ...
    public void drawPathEdges(ArrayList<Edge> pathEdges, GraphicsContext gc) {
        ...
        for (Edge edge : pathEdges) {
            ...
            if (costTime) {
                path = edge.getFromStop().getId() + ":" + edge.getToStop().getId() + "\t" + edge.getTripId()
                        + " \t Cost " + Math.round(edge.getCost()) + "s : to goal " + Transport.toTimeString(totalTime)
                        + "\n" + path;
            }else {
                path = edge.getFromStop().getId() + ":" + edge.getToStop().getId() + "\t" + edge.getTripId()
                        + " \t Cost " + Math.round(edge.getCost()) + "m : to goal " + Math.round(totalDistance)
                        + "\n" + path;
            }
        }
        // TODO: add total time to the path
        if (totalTime != 0 && costTime) {
            path += "\n" + " " + Transport.toTimeString(totalTime) +" min";
        } else if(totalDistance != 0 && !costTime){
    ...
    public void drawGraph(Graph graph) {
        ...
        // draw edges using a lambda functionh
